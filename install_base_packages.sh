...
printf "${YELLOW} Updating OS and Installing Base Packages.\n"
...
printf "${YELLOW}Reconfiguring SSH Server.\n"
sudo rm -v /etc/ssh/ssh_host_* && \
sudo ssh-keygen -f /etc/ssh/ssh_host_rsa_key -N '' -t rsa && \
sudo ssh-keygen -f /etc/ssh/ssh_host_dsa_key -N '' -t dsa && \
sudo ssh-keygen -f /etc/ssh/ssh_host_ecdsa_key -N '' -t ecdsa -b 521 && \

printf "${RED}It is recommend that you reboot the system now.\n"
printf "${Yellow}Remember to run ssh-keygen -R %hostname% on your workstation before connecting via ssh again.\n"